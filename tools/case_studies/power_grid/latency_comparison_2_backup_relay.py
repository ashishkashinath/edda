...
# ── Read data ────────────────────────────────────────────────────
df = pd.read_csv('latency_results_2_backup_relay.csv')
...
# ── Axes labels & ticks ──────────────────────────────────────────
ax.set_xlabel("Backup Relay", fontsize=28)
ax.set_ylabel("Worst-case Delay (ms)", fontsize=28)
#ax.set_xticks(x)
#ax.set_xticklabels(categories, fontsize=28)
ax.set_xticklabels([])
ax.tick_params(axis='y', labelsize=28)
ax.legend(fontsize=20, ncol=1, loc='upper right', bbox_to_anchor=(1.01, 1.01), frameon=True)
...
plt.tight_layout()
plt.savefig("latency_comparison_2_backup_relay.pdf", dpi=300, bbox_inches="tight")